...
sub regularPage {
    ...
    print "<form action='http://undcemcs02.und.edu/~mark.arinaitwe/cgi-bin/513/3/Assignment6JohnNeis.pl' method='post'>", "\n";
    print "<table>", "\n";
...
if(param()) {
    if(param("btnAdd")) {
        ...
        if($todoItem eq "") {
            print redirect("http://undcemcs02.und.edu/~mark.arinaitwe/cgi-bin/513/3/Assignment6JohnNeis.pl?flState=1");
            exit;
        }
        open(TODO, ">>todolist.txt") or die("Unable to open file.\n");
        print TODO $todoItem, "\n";
        close(TODO);
        print redirect("http://undcemcs02.und.edu/~mark.arinaitwe/cgi-bin/513/3/Assignment6JohnNeis.pl?addState=1");
